...
defaultMacAppBundleName = "eric4.app"
...
def usage(rcode = 2):
    ...
    if sys.platform == "darwin":
        print "    %s [-chxz] [-a dir] [-b dir] [-d dir] [-f file] [-i dir]" \
              " [-m name] [-p python]" % progName
    elif sys.platform.startswith("win"):
        print "    %s [-chxz] [-a dir] [-b dir] [-d dir] [-f file]" % progName
    else:
    ...
    if sys.platform.startswith("linux"):
        if distDir:
            ...
            if not os.path.exists(dst):
                os.makedirs(dst)
            shutilCopy(
                os.path.join(sourceDir, "icons", "default", "eric_2.png"),
                os.path.join(dst, "eric4.png"))
            dst = os.path.normpath(
                os.path.join(distDir, "usr/share/applications"))
            if not os.path.exists(dst):
                os.makedirs(dst)
            shutilCopy(os.path.join(sourceDir, "eric4.desktop"), dst)
            dst = os.path.normpath(
                os.path.join(distDir, "usr/share/appdata"))
            if not os.path.exists(dst):
                os.makedirs(dst)
            shutilCopy(os.path.join(sourceDir, "eric4.appdata.xml"), dst)
        elif os.getuid() == 0:
            shutilCopy(os.path.join(
                sourceDir, "icons", "default", "eric_2.png"),
                "/usr/share/pixmaps/eric4.png")
            shutilCopy(os.path.join(
                sourceDir, "eric4.desktop"),
                "/usr/share/applications")
            if os.path.exists("/usr/share/appdata"):
                shutilCopy(os.path.join(
                    sourceDir, "eric4.appdata.xml"),
                    "/usr/share/appdata")
...
def createMacAppBundle(pydir):
    ...
    dirs = {
        ...
        "exe": "%s/%s/Contents/MacOS" % (
            macAppBundlePath, macAppBundleName),
        "icns": "%s/%s/Contents/Resources" % (
            macAppBundlePath, macAppBundleName)
    }
...
''' % (macAppBundleName.replace(".app", ""), macAppBundleName.replace(".app", ""))
    )
...
_pkg_config = {
    ...
    'apidir'               : r'%s',
    'apis'                 : %s,
    'macAppBundlePath'     : r'%s',
    'macAppBundleName'     : r'%s',
}
