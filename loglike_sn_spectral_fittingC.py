...
spec.flux = spec.flux * (1 + float(sys.argv[2]))
spec.var = spec.var * (1 + float(sys.argv[2]))
...
for snt in snlist:
   for galt in gallist: 
    ...
    """We transform the galaxy and sn templates to rest-frame"""
    sntmpl.wavelength=sntmpl.wavelength / (1 + float(sys.argv[2]))
    sntmpl.flux=sntmpl.flux * (1 + float(sys.argv[2]))
    galtmpl.wavelength=sntmpl.wavelength / (1 + float(sys.argv[2]))
    galtmpl.flux=sntmpl.flux * (1 + float(sys.argv[2]))
...
"""Again conversion to rest-frame"""
bsnt.wavelength=bsnt.wavelength / (1 + float(sys.argv[2]))
bsnt.flux=bsnt.flux * (1 + float(sys.argv[2]))
bgalt.wavelength=bgalt.wavelength / (1 + float(sys.argv[2]))
bgalt.flux=bgalt.flux * (1 + float(sys.argv[2]))
